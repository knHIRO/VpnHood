...
public sealed class AppNotification : IDisposable
{
    ...
    public AppNotification(Context context)
    {
        ...
        var pendingFlags = OperatingSystem.IsAndroidVersionAtLeast(23) ? PendingIntentFlags.Immutable : 0;
        var pendingOpenIntent = PendingIntent.GetActivity(_context, 0, openIntent, pendingFlags);
    ...
    private PendingIntent CreatePendingIntent(string name)
    {
        ...
        var pendingFlags = OperatingSystem.IsAndroidVersionAtLeast(23) ? PendingIntentFlags.Immutable : 0;
        var pendingIntent = PendingIntent.GetBroadcast(_context, 0, intent, pendingFlags) ?? throw new Exception("Could not acquire Broadcast intent.");
